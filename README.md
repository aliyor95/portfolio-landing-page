...
и на статический хостинг (Devin / любой CDN).
...
2. Запушьте в ветку `main` — сайт соберётся и опубликуется на
   `https://aliyor95.github.io/portfolio-landing-page/`.

Базовый путь относительный (`base: './'` в `vite.config.ts`), поэтому сборка работает
и из корня домена, и из под-пути. Для абсолютного базового пути используйте
переменную окружения `BASE_PATH`.
